	...
	{                                                                                                     \
		...
		if(IPPU.DoubleHeightPixels && ((PPU.BGMode != 5 && PPU.BGMode != 6) || !IPPU.Interlace)) {        \
			for(y = GFX.StartY; y <= GFX.EndY; y++) {                                                     \
				...
				memcpy(SCREEN + (y * 2 + 1) * GFX.Pitch2, SCREEN + y * 2 * GFX.Pitch2, GFX.Pitch2);       \
				if(DO_DEPTH) {                                                                            \
...
#define RENDER_BACKGROUND_MODE7_i(TYPE, FUNC, COLORFUNC)                                                                                                                                                                                                         \
	...
	Depth = GFX.DB + GFX.StartY * GFX.PPL;                                                                                                                                                                                                                       \
	l = &LineMatrixData[GFX.StartY];                                                                                                                                                                                                                             \
	if(!l->MatrixB && !l->MatrixC && (l->MatrixA == 0x0100) && (l->MatrixD == 0x0100) && !LineMatrixData[GFX.EndY].MatrixB && !LineMatrixData[GFX.EndY].MatrixC && (LineMatrixData[GFX.EndY].MatrixA == 0x0100) && (LineMatrixData[GFX.EndY].MatrixD == 0x0100)) \
	...
	for(Line = GFX.StartY; Line <= GFX.EndY; Line++, Screen += GFX.Pitch, Depth += GFX.PPL, l++) {                                                                                                                                                               \
		...
		for(clip = 0; clip < ClipCount; clip++) {                                                                                                                                                                                                                \
			...
			if(simpleCase) {                                                                                                                                                                                                                                     \
				...
				} else /* The oversampling method: get the colors at four corners of a square */ /* in the _displayed_ image, and average them.  It's sharp and clean, but */ /* gives the usual huge pixels when the source image gets "close." */              \
				{                                                                                                                                                                                                                                                \
