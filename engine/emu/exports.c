...
void S9xSoundCallback(void) {
	...
	if(soundBufferOutPos >= 2048) {
		if(outToExternalBufferSamplePos <= 2048 && nextPos >= 2048) {
			return;
		}
	} else {
		if(outToExternalBufferSamplePos > 2048 && nextPos <= 2048) {
...
EMSCRIPTEN_KEEPALIVE
int startWithRom(unsigned char *rom, unsigned int romLength, unsigned int sampleRate) {
	if(!coreInited) {
	...
	if(!LoadROMFromBuffer(rom, romLength)) {
		runGameFlag = false;
		return 0;
	}
	S9xReset();
...
float *getSoundBuffer() {
	...
	if(!f32soundBuffer) {
		f32soundBuffer = (float *)calloc(2048 * 2, sizeof(float));
	}
	if(soundBufferOutPos < 2048) {
		if(outToExternalBufferSamplePos < 2048) {
			return f32soundBuffer; // getSoundBufferが呼ばれすぎてS9xSoundCallbackによって生成された音声データに追いついた
		}
	} else {
		if(outToExternalBufferSamplePos >= 2048) {
			return f32soundBuffer; // getSoundBufferが呼ばれすぎてS9xSoundCallbackによって生成された音声データに追いついた
	...
	for(unsigned int i = 0; i < 2048; i++) {
		for(unsigned int j = 0; j < 2; j++) {
			f32soundBuffer[j * 2048 + i] = outToExternalBuffer[(soundBufferOutPos * 2 + i * 2 + j) % 8192] / ((float)(0x8000));
		}
	}
